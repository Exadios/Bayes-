 ...
 *  bf_observe:  Observe SLAM feature
 */
		 ...
		 )
{
	...
	if (!usageOk)
		mexErrMsgTxt("usage: slam_observe(<handle>, <feature_no>, <Hx>, <z>, <Z_vector>)");
	...
	try
	{
		...
		// Observe model
		Bayesian_filter::Linear_uncorrelated_observe_model model(Hx.size2(), z.size());
		model.Hx = Hx;
		model.Zv = Zv;
